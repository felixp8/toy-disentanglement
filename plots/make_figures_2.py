...
def plot_compare(datas, path):
    families = [
        ("1D wavelet", datas[16], "#1f77b4"),
        ("2D wavelet", datas[21], "#d62728"),
        ("3D wavelet", datas[22], "#2ca02c"),
        ("4D wavelet", datas[23], "#d69327"),
        # ("5D wavelet", datas[24], "#6e2ca0"),
    ]
    fig, axes = plt.subplots(1, 2, figsize=(7.5, 3.1))
    for name, data, color in families:
        ...
        for P in P_VALUES:
            if (P, 5) in data:
                xs.append(P)
                r2s.append(data[(P, 5)]["r2"])
                accs.append(data[(P, 5)]["acc_ub"])
    ...
    fig.suptitle(r"Fixed depth $k = 5$", fontsize=11)
    fig.tight_layout()
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
...
def plot_fig3e_reproduction(data, path, depth=2):
    ...
    axes[0].set_xticks([5, 10, 20, 50]); axes[0].set_xticklabels(["5", "10", "20", "50"])
    axes[0].set_xlabel("# tasks $P$")
    axes[0].set_ylabel(r"classifier generalization classifier generalization\n(accuracy)")
    ...
    axes[1].set_xticks([5, 10, 20, 50]); axes[1].set_xticklabels(["5", "10", "20", "50"])
    axes[1].set_xlabel("# tasks $P$")
    axes[1].set_ylabel(r"regression generalization regression generalization\n($R^2$)")
...
def main():
    ...
    datas = {sid: pull_sweep(sid) for sid in (16, 21, 22, 23)}
    for sid, d in datas.items():
        print(f"  sweep {sid}: {len(d)} cells")
    
    plot_family(datas[16], "1D wavelet", FIGS / "1d_wavelet.pdf")
    plot_family(datas[21], "2D wavelet", FIGS / "2d_wavelet.pdf")
    plot_family(datas[22], "3D wavelet", FIGS / "3d_wavelet.pdf")
    plot_family(datas[23], "4D wavelet", FIGS / "4d_wavelet.pdf")
    # plot_family(datas[24], "5D wavelet", FIGS / "5d_wavelet.pdf")
    plot_compare(datas, FIGS / "wavelet_compare.pdf")
